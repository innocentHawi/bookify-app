import { Account, Avatars, Client, Databases, ID, Query, Storage, Functions } from 'react-native-appwrite';
...
export const getCurrentUser = async () => {
    try {
        ...
        //console.log("Error in getCurrentUserR:", error.message);
    }
}
...
export const fetchSubscriptionStatus = async (readerId) => {
    try {
        ...
        if (result.documents.length === 0) {
            console.error("No matching document found for readerId:", readerId);
            return false;
        }

        // Directly return the subscriptionStatus
        return result.documents[0].subscriptionStatus ?? false; 
    } catch (error) {
